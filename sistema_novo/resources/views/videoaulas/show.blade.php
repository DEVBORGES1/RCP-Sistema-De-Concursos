...
<div class="container">
    <div class="header-content" style="display: flex;justify-content: space-between;align-items: center;margin-bottom: 30px;">
        <h1 style="font-size: 1.8rem;color: var(--dash-text-primary, white);"><i class="{{ $categoria->icone }}"></i> {{ $categoria->nome }}</h1>
        <div class="user-info">
    ...
    @if (empty($temas))
        <div class="empty-state" style="text-align: center; padding: 60px 20px; color: var(--dash-text-secondary, #7f8c8d);">
            ...
            <h3 style="margin: 0 0 10px 0; color: var(--dash-text-primary, white);">Nenhuma disciplina encontrada</h3>
            <p style="color: var(--dash-text-secondary, #ccc);">Ainda não há videoaulas cadastradas para esta matéria.</p>
        </div>
    @else
        <div class="temas-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 25px;">
            @foreach ($temas as $tema)
                <div class="tema-card {{ $tema['progresso'] == 100 ? 'completo' : '' }}" 
                     style="background: var(--dash-bg-card, rgba(255,255,255,0.05)); border-radius: 15px; padding: 25px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); transition: all 0.3s ease; border: 1px solid var(--dash-border, transparent); position: relative; overflow: hidden; --tema-cor: {{ $categoria->cor }}">
                    ...
                    <div class="tema-header" style="display: flex; align-items: center; margin-bottom: 15px;">
                        ...
                        <div class="tema-info">
                            <h3 style="margin: 0; color: var(--dash-text-primary, white); font-size: 1.3em; font-weight: bold;">{{ $tema['nome'] }}</h3>
                            <p style="margin: 5px 0 0 0; color: var(--dash-text-secondary, #ccc); font-size: 0.9em;">{{ count($tema['videoaulas']) > 0 ? count($tema['videoaulas']) . ' videoaula(s)' : 'Aguardando videoaulas' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="tema-progress" style="margin: 15px 0;">
                        <div class="progress-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                            <span class="progress-label" style="font-size: 0.9em; font-weight: 600; color: var(--dash-text-primary, white);">Progresso</span>
                            <span class="progress-percentage" style="font-weight: bold; color: {{ $categoria->cor }}; font-size: 1em;">{{ $tema['progresso'] }}%</span>
                        </div>
                        <div class="progress-bar" style="width: 100%; height: 10px; background: var(--dash-border, rgba(255,255,255,0.1)); border-radius: 10px; overflow: hidden;">
                    ...
                    <div class="aulas-list" style="margin-top: 15px; border-top: 1px solid var(--dash-border, rgba(255,255,255,0.1)); padding-top: 10px;">
                         @foreach ($tema['videoaulas'] as $aula)
                            <a href="{{ route('videoaulas.player', $aula->id) }}" class="aula-link" style="display: block; padding: 8px; color: var(--dash-text-secondary, #ccc); text-decoration: none; font-size: 0.9rem; border-radius: 5px; margin-bottom: 5px;">
                    ...
                    <div class="tema-stats" style="display: flex; justify-content: space-around; margin-top: 15px; padding-top: 15px; border-top: 1px solid var(--dash-border, rgba(255,255,255,0.1));">
                        <div class="stat-item" style="text-align: center;">
                            <h4 style="margin: 0; color: {{ $categoria->cor }}; font-size: 1.3em; font-weight: bold;">{{ $tema['total'] }}</h4>
                            <p style="margin: 3px 0 0 0; color: var(--dash-text-secondary, #ccc); font-size: 0.8em;">Total</p>
                        </div>
                        <div class="stat-item" style="text-align: center;">
                            <h4 style="margin: 0; color: {{ $categoria->cor }}; font-size: 1.3em; font-weight: bold;">{{ $tema['concluidas'] }}</h4>
                            <p style="margin: 3px 0 0 0; color: var(--dash-text-secondary, #ccc); font-size: 0.8em;">Concluídas</p>
                        </div>
                    </div>
                </div>
            @endforeach
